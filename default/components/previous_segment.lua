...
return {
	...
	widget = function()
		...
		if snapshot.current_split and snapshot.current_split > 1 then
			delta_number = time.live_delta(snapshot.current_split - 1)
		elseif snapshot.current_phase == "Ended" then
			delta_number = time.live_delta(#run.segments)
		end
