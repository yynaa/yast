...
local function plugin(text_name, color, align_x, align_y, show_if)
	return function(settings)
		local s =
			settings:string(text_name .. " Text: Font", "", show_if):number(text_name .. " Text: Size", 12, show_if)

		if color then
			s = s:color(text_name .. " Text: Color", 1, 1, 1, 1, show_if)
		end
		if align_x then
...
local function write(text, text_name)
	return widgets.text(text):font(font(text_name)):size(size(text_name))
end
