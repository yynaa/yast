...
local function plugin(settings)
	return settings
		...
		:color("Background: Solid Color", 1, 1, 1, 1, function(s)
			return s("Background: Type") == "Solid"
		end)
		:image("Background: Image", function(s)
			return s("Background: Type") == "Image"
		end)
		:number_range("Background: Image Opacity", 0, 100, 1, 0, function(s)
			return s("Background: Type") == "Image"
		end)
end
		...
		return widgets
			.stack({
				widgets
					...
					:height("fill")
					:content_fit("cover")
					:opacity(setting("Background: Image Opacity") / 100)
					:into(),
				widget,
			})
			:width("fill")
